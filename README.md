...
<p>このリポジトリは、shellscriptを実行して引数に指定したファイルの文字列を読み取ってフォルダを作成します</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、shellscriptを実行してフォルダが作成されていることを確認してください</p>
...
<div class="wp-block-group">
<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>git clone https://github.com/trygfmi/shellscript_make-folders-from-text
...
<pre class="wp-block-code has-background" style="background-color:#ffeeee"><code>hello
shellscript
world</code></pre>
</details>
</div>
...
<div class="wp-block-group">
<p>以下のコマンドを端末で実行することで詳細のようなフォルダが作成されているはずです</p>
...
<pre class="wp-block-code has-24292-eff-color has-text-color has-background has-1-125-rem-font-size" style="background-color:#ffeeee"><code>hello
shellscript
world</code></pre>
</details>
</div>
...
<div class="wp-block-group">
<p>以下のコマンドをターミナルで実行することで詳細のようなフォルダが作成されているはずです</p>
...
<div class="wp-block-group">
<p>以下のコマンドをプロンプトで実行することで詳細のようなフォルダが作成されているはずです</p>
...
<div class="wp-block-group">
<p>shellscriptの引数でサブシェルを実行して"$@"で全ての引数を取得しています<br>ダブルクオーテーションは空白が含まれていても一つの文字列として扱うためにつけています</p>
